...
function Player: checkLeftCollisions(dt)
  ...
  if (tileTopLeft and tileTopCottomLeft) and (TileTopLeft:collidable() or tileBottomLeft:collidable()) then
    self.x = (tileTopLeft.x - 1) * TILE_SIZE + tileTopLeft.width - 1
  else

    self.y = self.y - 1
    local collidableObljects = self:checkObjectCollisions()
    self.y = self.y + 1
...
function Player:checkObjectCollisions()
  ...
  for k, object in pairs(self.level.objects) do
    if object:collides(self) then
      ...
      elseif object.comsummable then
